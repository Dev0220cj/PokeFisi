...
| `DAMAGE_K` | 0.1 | Factor velocidad en fórmula de daño |
...
Heurística básica basada en HP (según enunciado del curso).
- Score de movimiento: `poder × efectividad × STAB × (precision/100)`
- Movimientos de estado: score fijo bajo (no es el foco)
- Cambia de Pokémon solo si HP propio < 30 %, eligiendo candidato con más HP relativo y menor vulnerabilidad de tipo
- **No** tiene KO awareness ni estimación real de daño ni cambio estratégico avanzado — esas eran complejidades innecesarias para Nivel 2 que pueden moverse al Nivel 3 (Minimax) vía su función `evaluar()` y lookahead
...
- Función de evaluación compuesta (4 factores con pesos configurables):
  ...
  - `type_advantage`: mejor efectividad de tipo del activo, normalizada a [-1, 1]
...
4. Escalar cada frame con `pygame.transform.smoothscale(frame, tamaño)` — bilineal, mejor calidad que NEAREST para pixel art ampliado
...
| 15 | `battle_engine` | Sin movimiento Struggle cuando todos los movimientos tienen 0 PP |
| 16 | `battle_engine` | Sin varianza de daño ni golpes críticos |
| 18 | `battle_engine` | Cometa Draco no aplica reducción de SpAtk propia tras usarse |
...
- **Decisión**: revertir a una heurística básica según el enunciado:
  - Score de movimiento: `poder × efectividad × STAB × (precision/100)` (sin estimar daño real)
  - Cambio solo si HP < 30 %, con score por HP del candidato + vulnerabilidad de tipo simple
  - Sin KO awareness, sin matchup awareness avanzado
...
- Todos los factores en [-1, 1] para evitar saturación entre términos.
